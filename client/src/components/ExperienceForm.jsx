...
const ExperienceForm = ({ data, onChange }) => {
   ...
   const removeExperience = (index) => {
      const updated = data.filter((item, i) => i !== index);
      onChange(updated);
   }
                  ...
                  <div key={index} className='p-4 border border-gray-200 rounded-lg space-y-3'>
                     ...
                     <div className='grid md:grid-cols-2 gap-3'>
                        <input value={experience.company || ""} onChange={(e) => updateExperience(index, "company", e.target.value)} type="text"
                           className='px-3 py-2 text-sm rounded-lg' placeholder='Company Name' />
                        <input value={experience.position || ""} onChange={(e) => updateExperience(index, "position", e.target.value)} type="text"
                           className='px-3 py-2 text-sm rounded-lg' placeholder='Job Title' />
                     ...
                     <label className='flex items-center gap-2'>
                        <input type="checkbox" checked={experience.is_current || false} className='rounded border-gray-300 text-blue-600 focus:ring-blue-500'
