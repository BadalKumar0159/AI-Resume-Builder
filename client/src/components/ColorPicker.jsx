...
const ColorPicker = ({ onChange, selectedColor }) => {
    ...
    return (
        <div className='relative'>
            <button onClick={() => setIsOpen(prev => !prev)} className='flex items-center gap-1 text-sm text-purple-600 bg-gradient-to-br 
            from-purple-50 to-purple-100 hover:ring ring-purple-300 px-3 py-2 rounded-lg transition-all'>
                <Palette size={16} /> <span className='max-sm:hidden'>Accent</span>
            </button>

            {isOpen && (
                <div className="grid grid-cols-4 w-60 gap-2 absolute top-full left-0 p-3 mt-2 z-10 bg-white 
